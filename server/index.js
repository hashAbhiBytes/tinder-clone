...
app.post("/signup", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ user_id: userId, email }, jwtSecret, {
      expiresIn: "1d",
    });
...
app.post("/login", async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ user_id: user.user_id, email }, jwtSecret, {
      expiresIn: "1d",
    });
  ...
  try {
    ...
    if (user && user.matches) {
      const matchedUsers = await users
        .find({ user_id: { $in: user.matches.map((match) => match.user_id) } })
        .toArray();
