...
const Dashboard = () => {
  ...
  const getChatMessages = async (matchId) => {
    try {
      ...
      const response = await axios.get("http://localhost:8000/messages", {
        params: { userId, correspondingUserId: matchId },
      });
      ...
      const receivedMessages = receivedResponse.data;
  ...
  useEffect(() => {
    if (activeTab === "chat" && currentMatch) {
      getChatMessages(currentMatch.user_id);
      //   getMatchUser(match.user_id);
  ...
  const swipeLeft = () => {
    if (
      ...
      currentIndex >= filteredGenderedUsers.length
    )
      return;
                    ...
                    filteredGenderedUsers.map((profile, index) => (
                      <TinderCardContainer
                        ...
                        className={`swipe-card ${
                          index !== currentIndex ? "hidden-card" : ""
                        }`}
                        key={profile.user_id}
                      ...
                      >
                        <div
                          ...
                          style={{
                            backgroundImage: `url(${
                              profile.url || "https://via.placeholder.com/400"
                            })`,
                          }}
                  ...
                  {user.matches && user.matches.length > 0 ? (
                    user.matches.map((match) => (
                      <div
                        ...
                        onClick={() => {
                          //   const curmatch = getUser(match.user_id);
                          setActiveTab("chat");
                      ...
                      >
                        <div
                          ...
                          style={{
                            backgroundImage: `url(${
                              match.url || "https://via.placeholder.com/150"
                            })`,
                        ...
                        <div className="match-info">
                          ...
                          {match.about && (
                            <p className="match-bio">
                              {match.about.length > 30
                                ? `${match.about.substring(0, 30)}...`
                                : match.about}
                            </p>
                          )}
                        </div>
                  ...
                  {chatMessages.length > 0 ? (
                    chatMessages.map((message, index) => (
                      <div
                        key={index}
                        className={`message ${
                          message.from_userId === userId ? "sent" : "received"
                        }`}
                      >
                        <p>{message.message}</p>
              ...
              <div className="profile-section">
                <div className="profile-card large">
                  <div
                    ...
                    style={{
                      backgroundImage: `url(${
                        user.url || "https://via.placeholder.com/400"
                      })`,
                    }}
          ...
          {matchAnimation && (
            <div className="match-animation">
              <div className="match-content">
                <h2>It's a Match!</h2>
                <p>
                  You and {filteredGenderedUsers[currentIndex - 1]?.first_name}{" "}
                  liked each other
                </p>
                <div className="match-avatars">
                  ...
                  <div className="avatar-container">
                    <img
                      src={
                        filteredGenderedUsers[currentIndex - 1]?.url ||
                ...
                <div className="match-actions">
                  <button
                    ...
                    onClick={() => {
                      ...
                      setMatchAnimation(false);
                      getChatMessages(
                        filteredGenderedUsers[currentIndex - 1]?.user_id
                      );
                    }}
                    aria-label="Send message"
                  >
